...
def test_catalog(catalog):
    ...
    assert namespace["default"] is False
...
def test_create_order_and_preview(catalog, apply):
    ...
    if not apply:
        ...
        assert len(calls) == 20
        assert calls[0].kwargs["kind"] == "IpamNamespace"
        assert [c.kwargs["data"]["prefix"] for c in calls[1:]] == [
            p["prefix"] for p in prefixes
        ]
        assert all(c.kwargs["branch"] == "seed-test" for c in calls)
        assert all(
            c.kwargs["data"]["ip_namespace"] == "namespace-id" for c in calls[1:]
        )
...
def test_matching_rerun_and_namespace_isolation(catalog):
    namespace, prefixes = catalog
    client = MagicMock()
    client.get.side_effect = [node(namespace, id="global-id")] + [
        node(p, vrf=SimpleNamespace(id=None)) for p in prefixes
    ]
    assert module.seed(client, "seed-test", namespace, prefixes, True) == 0
    client.create.assert_not_called()
    assert all(
        call.kwargs["hfid"][0] == "global" for call in client.get.call_args_list[1:]
    )
